...
class Game:
    ...
    def print_statistics(self):
        # печатать должен код вызывающий этот метод, если нужно
    ...
    def parse_info(self, element):
        if 'Location' in element:
            ...
            elem_name = 'Boss'
            if not re.sub(r'Boss_exp\d+_tm[0-9.]{1,}', '', element):
    ...
    def make_choice(self, num):
        ...
        while not choice_done:
            ...
            for j in sorted(self.num2action):
                print(j, ' ', self.num2action[j], self.num2name[j], ', займет:', self.num2time[j],
                      ', будет заработано:', self.num2exp[j], 'опыта')
            exit_num = max(self.num2action.keys())+1
            print(exit_num, ' ', 'сдаться и выйти из игры')
            user_choice = input(f'\nвведите цифру в соответствии с предложенными вариантами\n')
            if user_choice.isnumeric():
                # выбрал выход
                if int(user_choice)==exit_num:
                    # запишем инфо в лог
                    self.write_info_to_csv('user_exit', self.total_user_exp,
                                           str(timedelta(seconds=self.seconds_from_start)))
                    choice_done = True
                # выбрал один из вариантов
                elif int(user_choice) in self.num2action.keys():
                    # запишем инфо в лог
                    self.write_info_to_csv(self.num2elem[int(user_choice)], self.total_user_exp,
                                           str(timedelta(seconds=self.seconds_from_start)))
                    choice_done = True
                else:
    ...
    def run(self, json_element=None):
        try:
            ...
            if not json_element:
                self.print_statistics()  # показываю статистику при каждом вод в локацию
                json_element = self.start_location
                # запишем инфо в лог
                self.write_info_to_csv(json_element, self.total_user_exp,
                                       str(timedelta(seconds=self.seconds_from_start)))
            ...
            for elem in self.new_data:
                num += 1
                if isinstance(elem, dict):
                    element = list(elem.keys())[0]
                    self.num2action[num] = 'перейти в локацию:'
                else:
                    element = elem
                    self.num2action[num] = 'сразиться с:'

                elem_name, elem_seconds, elem_time, elem_exp = self.parse_info(element) # норм, каждый хранит свои данные,
                # уже приведенные в нужный вид, знаю откуда что можно дернуть, мне удобно, да и места они особо не кушают
            ...
            # обнулим накопленные за игру значения
            self.remaining_time = float('123456.0987654321')
            self.seconds_from_start = float(0)  # время в игре
            self.total_user_exp = 0  # накопительный опыт пользователя
            self.mosters_killed = 0  # количество убитых монстров
            # сбросим уровень json до стартового
