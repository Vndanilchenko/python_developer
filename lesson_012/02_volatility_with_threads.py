...
class CalculateVolatility(Thread):
    ...
    def __init__(self, path, filename, thread_num, *args, **kwargs):
    ...
    def run(self):
        # print('\nthread №', self.thread_num, ': STARTED', flush=False)
        self.read_file(self.path)
        self.calculate_volatility()
        # print('\nthread №', self.thread_num, ': FINISHED', flush=False)
...
def main():
    ...
    volatilities = {}
    objs = [CalculateVolatility(path, filename, i) for i, filename in enumerate(os.listdir(path))]
    for obj in objs:
        obj.start()
    for obj in objs:
        obj.join()
